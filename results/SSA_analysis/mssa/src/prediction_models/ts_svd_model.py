...
class SVDModel(object):
    ...
    def _computeWeights(self):       
        ...
        if len(self.lastRowObservations) >= 2*self.no_ts:
            for i in range(self.no_ts):
                # Jiayu:
                self.forecast_model_score[i] = r2_score(self.lastRowObservations[int(i*80):int((i+1)*80)]/self.p, np.dot(matrix[:,int(i*80):int((i+1)*80)].T,self.weights))
    ...
    def denoisedTS(self, ind = None, range = True,return_ = True, ts = None):
        ...
        if ts is None:    
            ...
            # Jiayu:
            NewColsDenoised = self.matrix[:,int(ts*80):int((ts+1)*80)].flatten('F')
    ...
    def fit(self, keyToSeriesDF):
        ...
        for i in range(self.no_ts):
            # Jiayu:
            obs = obs_matrix[:,int(i*80):int((i+1)*80)].flatten('F')
            # obs = obs_matrix[:,i::self.no_ts].flatten('F') # 每隔no_ts列取一列，对应 _assignData
