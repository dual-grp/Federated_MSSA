...
from FLAlgorithms.users.userLR import UserLR
...
class serverLR(Server):
    def __init__(self, experiment, device, dataset,algorithm, model, batch_size, learning_rate, beta, L_k, num_glob_iters, local_epochs, optimizer, fac_users, times , cutoff, mulTS, missingVal, numusers, datatype):
        ...
        for i in range(self.total_users):
            ...
            user = UserLR(device, id, train, test, model, batch_size, learning_rate,beta,L_k, local_epochs, optimizer)
            print(user.id)
    ...
    def train(self):
        ...
        # self.save_results()
        model_name = f"FedLR_{self.dataset}_num_user_{self.total_users}_L_80_dim_40_MP_{self.missingVal}_W79"
        if self.mulTS == 0:
