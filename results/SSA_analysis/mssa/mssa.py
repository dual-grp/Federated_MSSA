    ...
    def _get_imputation_range_local(self, t1, t2, model, ts_no):
        ...
        if m1 == m2:
            ...
            for m in range(m1, m2 + 1 + (m2 < last_model)):
                ...
                S = model.models[m].sk[:]
                # Jiayu:
                V = model.models[m].Vk[int(ts_no*80):int((ts_no+1)*80)][tscol_i:tscol_f + 1, :]
